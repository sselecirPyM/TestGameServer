...
namespace Simulation
{
    public abstract class ReadOnlyKDTree3<T>
    {
        ...
        public abstract bool Any(Vector3 position, float radius, Predicate<T> filter);
    }
    ...
    public sealed class KDTree3<T> : ReadOnlyKDTree3<T>
    {
        struct BB
        {
            public Vector3 min;
        ...
        public void Build()
        {
            ...
            if (values.Count == 0)
            {
                root = new Tree3Node() { splitType = SplitType.Leaf };
                return;
            }
        ...
        public override bool Any(Vector3 position, float radius, Predicate<T> filter)
        {
            return _Any(root, position, radius, radius * radius, filter);
        }
        ...
        bool _Any(Tree3Node node, Vector3 position, float radius, float r2, Predicate<T> condition)
        {
            if (node.childCount == 0)
            {
                return false;
            ...
            if (node.splitType == SplitType.Leaf)
            {
                ...
                {
                    var v = values[i];
                    if (Vector3.DistanceSquared(position, v.position) <= r2 && condition(v.value))
                    {
                        return true;
                    }
                }
            ...
            else
            {
                ...
                if (l <= node.midValue && _Any(node.childL, position, radius, r2, condition))
                {
                    return true;
                }
                if (r >= node.midValue && _Any(node.childR, position, radius, r2, condition))
                {
                    return true;
                }
            }
