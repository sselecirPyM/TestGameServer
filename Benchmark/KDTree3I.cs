...
{
    public struct Tree3NodeIndex
    {
        public Vector3 position;
        public int index;
    }
    public sealed class KDTree3I<T> : ReadOnlyKDTree3<T>
    {
        struct BB
        {
            public Vector3 min;
        ...
        public List<Tree3NodeIndex> values = new List<Tree3NodeIndex>();
        public List<T> actualValue = new List<T>();

        public void Add(T value, Vector3 position)
        {
            values.Add(new Tree3NodeIndex()
            {
                position = position,
                index = actualValue.Count
            });
        ...
        public override void Nearest(Vector3 position, float maxDistance, Predicate<T> filter, Action<T> action)
        {
            ...
            if (resultIndex >= 0)
            {
                action(actualValue[values[resultIndex].index]);
            }
        }
        ...
        void _Nearest(Tree3Node node, Vector3 position, Predicate<T> filter, ref int result, ref float md, ref float md2)
        {
            if (node.splitType == SplitType.Leaf)
            {
                for (int i = node.left; i < node.right; i++)
                {
                    ...
                    if (xd2 <= md2 && filter(actualValue[v.index]))
                    {
                        result = i;
                        md2 = xd2;
                        if (xd2 > 0)
    ...
    public sealed class KDTree3I2<T> : ReadOnlyKDTree3<T>
    {
        struct BB
        {
            public Vector3 min;
        ...
        public List<Tree3NodeS> tree3NodeS = new List<Tree3NodeS>();
        public List<Tree3NodeIndex> values = new List<Tree3NodeIndex>();
        public List<T> actualValue = new List<T>();
        ...
        public void Build()
        {
            ...
            _Build(0, values.Count, 0, new BB() { min = min, max = max });
        }

        public override void ForRange(Vector3 position, float radius, Action<T> action)
        {
            _Search(tree3NodeS.Count - 1, position, radius, radius * radius, action);
        }

        public override void ForRange(Vector3 min, Vector3 max, Action<T> action)
        {
            _Search(tree3NodeS.Count - 1, new BB() { min = Vector3.Min(min, max), max = Vector3.Max(min, max), }, action);
        }
        ...
        public override void Nearest(Vector3 position, float maxDistance, Predicate<T> filter, Action<T> action)
        {
            ...
            float md = maxDistance;
            float md2 = md * md;
            _Nearest(tree3NodeS.Count - 1, position, filter, ref resultIndex, ref md, ref md2);
            if (resultIndex >= 0)
            {
                action(actualValue[values[resultIndex].index]);
            }
        ...
        void _Search(int index, Vector3 position, float radius, float r2, Action<T> action)
        {
            var node = tree3NodeS[index];
            if (node.childCount == 0)
            {
                return;
            }
        ...
        void _Search(int index, BB bb, Action<T> action)
        {
            var node = tree3NodeS[index];
            if (node.childCount == 0)
            {
        ...
        void _Nearest(int index, Vector3 position, Predicate<T> filter, ref int result, ref float md, ref float md2)
        {
            var node = tree3NodeS[index];
            if (node.splitType == SplitType.Leaf)
            {
        ...
        int _Build(int left, int right, int deep, in BB bb)
        {
            if (right - left < 12 || deep > 8)
            {
                var child = new Tree3NodeS()
                {
                    left = left,
                    right = right,
                    childCount = right - left,
                ...
                tree3NodeS.Add(child);
                return tree3NodeS.Count - 1;
            }
        ...
        int _AddChild(int left, int right, int m, float midValue, int deep, SplitType splitType, in BB bb)
        {
            BB bb0 = bb;
            BB bb1 = bb;
            switch (splitType)
